...
fn _gpu_unsafe_naive_gemm2[
    ...
    if C_row < M and C_column < N:
        for k in range(K):
            partial += A_buffer[C_row * lda + k] * B_buffer[k * ldb + C_column]

        scaled_c = C_buffer[C_row * N + C_column] * b + partial * a
        C_buffer[C_row * ldc + C_column] = scaled_c
...
fn gpu_unsafe_naive_gemm2[
    ...
    ctx.enqueue_function[_gpu_unsafe_naive_gemm2[dtype]](
        A_buffer,
        B_Buffer,
        C_Buffer,
        M,
